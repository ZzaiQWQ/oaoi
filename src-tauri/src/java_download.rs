...
fn do_download_java(app_handle: &tauri::AppHandle, major: u32, game_dir: &str) -> Result<String, String> {
    ...
    /// 流式下载：边读边写磁盘
    fn stream_download(http: &reqwest::blocking::Client, url: &str, dest: &std::path::Path) -> Result<u64, String> {
    ...
    let dl_size = match stream_download(&http, &official_url, &tmp_zip) {
        Ok(size) => size,
        Err(_) => {
            ...
            stream_download(&http, &download_url, &tmp_zip)?
        }
    };
    eprintln!("[java] 下载完成 ({:.1} MB)，解压中...", dl_size as f64 / 1048576.0);
...
pub fn download_java_sync(major: u32, game_dir: &str) -> Result<String, String> {
    let java_exe = std::path::PathBuf::from(game_dir)
        .join("runtime").join(format!("jre-{}", major)).join("bin").join("java.exe");
    if java_exe.exists() {
        return Ok(java_exe.to_string_lossy().to_string());
    }
    let java_dir = std::path::PathBuf::from(game_dir).join("runtime").join(format!("jre-{}", major));
    std::fs::create_dir_all(&java_dir).map_err(|e| e.to_string())?;

    let official = format!(
        "https://api.adoptium.net/v3/binary/latest/{}/ga/windows/x64/jre/hotspot/normal/eclipse", major
    );
    let mirror_base = format!(
        "https://mirrors.tuna.tsinghua.edu.cn/Adoptium/{}/jre/x64/windows/", major
    );
    let http = reqwest::blocking::Client::builder()
        .timeout(std::time::Duration::from_secs(300))
        .user_agent("OAOI-Launcher/1.0")
    ...
    // 2. 官方失败 → 解析清华镜像目录页找到真实文件名
    if !downloaded {
        eprintln!("[java-sync] 官方源失败，尝试清华镜像...");
        if let Ok(resp) = http.get(&mirror_base).send() {
            if let Ok(listing) = resp.text() {
                let zip_name = listing.lines().filter_map(|line| {
                    if let Some(start) = line.find("href=\"") {
                        let rest = &line[start + 6..];
                        if let Some(end) = rest.find('"') {
                            let name = &rest[..end];
    ...
    if !downloaded { return Err(format!("无法下载 Java {}", major)); }
    ...
    fn find_exe(dir: &std::path::Path) -> Option<std::path::PathBuf> {
        for e in std::fs::read_dir(dir).ok()?.flatten() {
            let p = e.path();
            if p.is_dir() { if let Some(r) = find_exe(&p) { return Some(r); } }
            else if p.file_name().map(|n| n == "java.exe").unwrap_or(false) { return Some(p); }
        }
        None
    }
    find_exe(&java_dir).map(|p| p.to_string_lossy().to_string())
