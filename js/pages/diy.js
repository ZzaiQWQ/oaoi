...
function initDiyPage() {
  ...
  // ====== 字体大小 ======
  const fontSlider = document.getElementById('diyFontSize');
  const fontVal = document.getElementById('diyFontSizeVal');
  if (fontSlider && fontVal) {
    fontSlider.value = getSetting('fontSize');
    fontVal.textContent = fontSlider.value + 'px';
    fontSlider.addEventListener('input', () => {
      fontVal.textContent = fontSlider.value + 'px';
      setSetting('fontSize', fontSlider.value);
  ...
  function applySakuraCount(count) {
    ...
    if (typeof SakuraPetals !== 'undefined') {
      new SakuraPetals(container, count);
    }
    setTimeout(() => {
  ...
  function applyPetalStyle(style) {
    ...
    // 让新创建的花瓣也用这个样式
    if (typeof SakuraPetals !== 'undefined') {
      SakuraPetals.prototype.petals = emojis;
    }
  }
  ...
  function applyOpacity(val) {
    ...
    if (launcher) launcher.style.background = `rgba(255,255,255,${val / 100})`;
  }
  ...
  function applyRadius(val) {
    ...
    if (globalBg) {
      globalBg.style.borderRadius = val + 'px';
      globalBg.style.overflow = 'hidden';
    }
  }
