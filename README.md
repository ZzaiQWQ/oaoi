...
安装时会自动下载游戏本体、依赖库、资源文件和对应 Loader 文件。版本文件会按官方 Minecraft 目录结构保存，方便实例、库文件和资源文件复用。

### 下载器

oaoi 使用统一下载器处理游戏文件、依赖库、资源文件、Loader 安装器、整合包内部文件和在线 Mod 下载。
...
下载源会按当前任务的来源策略排序。官方下载优先使用官方源，镜像下载优先使用镜像源，失败后再回退到可用候选源。
...
Java 8、Java 17、Java 21 等常用运行环境可以按需准备。Java 7 不会自动下载，低版本实例需要玩家手动安装 Java 7，并在版本设置里选择对应的 `java.exe`。
...
导入后会生成独立实例，并把 Mod、配置、材质包、光影包、存档等文件放到对应目录。安装过程中如果取消或失败，会清理未完成的实例目录，避免留下半成品。

整合包内部下载会保留整合包原有的兼容逻辑，只替换实际下载方法。解析出的 Mod、材质包、光影包等文件会交给统一下载器处理。
...
大整合包安装时可以看到当前正在处理的文件类型和数量。取消任务后会中断下载器里的相关任务，并停止后续安装流程。
